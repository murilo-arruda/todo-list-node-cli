...
function showTodos() {
  todos.forEach((todo, index) => {
    ...
    console.log(
      color(`${index} - [${todo.isChecked ? "X" : " "}] ${todo.text}`)
    );
  });
}
...
function showHelp() {
  ...
  ${chalk.green("add")} - add a new todo. Example ${chalk.inverse(
    "add my new task"
  )}\n
  ${chalk.blue("check")} - checkmark the  items. Example: ${chalk.inverse(
    "check 0 2"
  )}. this will check the first item and the third.\n
  ${chalk.red("remove")} - remove items from the list. Example ${chalk.inverse(
    "remove 0 1"
